...
function ret = d_loss_by_d_model(model, data, wd_coefficient)
  ...
  inputs = data.inputs;
  hidden = logistic(model.input_to_hid * inputs);
  last_layer_input = model.hid_to_class * hidden;
  
  class_normalizer = log_sum_exp_over_rows(last_layer_input);
  log_class_prob = last_layer_input - repmat(class_normalizer, [size(last_layer_input, 1), 1]); 
  output = exp(log_class_prob); 
  ...
  m = size(inputs, 2);   
  ret.hid_to_class = (wd_coefficient*model.hid_to_class) + ((output - t)*hidden')/m;
  ret.input_to_hid = (wd_coefficient*model.input_to_hid) + (model.hid_to_class'*(output - t) .* (hidden-hidden.^2) * (inputs'))/m;
